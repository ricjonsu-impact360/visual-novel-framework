/**
 * Character Helper Tool - IIFE Version
 * Version: 1.1.2
 * 
...
(function () {
    ...
    function initializeHelperTool () {
        ...
        window.CharacterHelperTool = {
            ...
            loadData: function () {
                try {
                    ...
                    this.emotions = (window._DATAGAME.listEmotion || []).slice().sort();
                    this.animations = (window._DATAGAME.pose || []).slice().sort();
                    // Handheld items are stored as an object, so extract keys
                    this.handheldItems = (window._DATAGAME.objectHandheld ? Object.keys(window._DATAGAME.objectHandheld).sort() : []);
                    return true;
                } catch (error) {
            ...
            fetchCurrentState: function () {
                ...
                try {
                    ...
                    if (charSprite.emotionNow !== undefined && charSprite.emotionNow >= 0) {
                        ...
                        var originalEmotionList = window._DATAGAME.listEmotion || [];
            ...
            applyEmotion: function () {
                ...
                try {
                    ...
                    var emotionIndex = (window._DATAGAME.listEmotion || []).indexOf(emotion);
            ...
            applyAnimation: function () {
                ...
                try {
                    ...
                    if (emotion) {
                        var emotionIndex = (window._DATAGAME.listEmotion || []).indexOf(emotion);
                        if (emotionIndex !== -1) {
                            charSprite.changeEmotion(emotionIndex);
            ...
            // All event listeners are now bound here
            bindEvents: function () {
                var self = this;
        ...
        window.beginHelper = function () {
            window.CharacterHelperTool.open();
        };

        window.endHelper = function () {
            window.CharacterHelperTool.close();
        };

        document.addEventListener('keydown', function (e) {
            if (e.ctrlKey && e.shiftKey && (e.key === 'D' || e.keyCode === 68)) {
                e.preventDefault();
                if (window.CharacterHelperTool.isOpen) {
                    window.endHelper();
                } else {
                    window.beginHelper();
                }
        ...
        if (_AUTO_SHOW) {
            ...
            setTimeout(function () { window.beginHelper();}, _SHOW_DELAY);
        }
    }
