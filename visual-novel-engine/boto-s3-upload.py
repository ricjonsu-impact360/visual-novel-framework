...
BUCKET_NAME = 'marketjs-visual-novel'
LANGUAGE_CODE = None
...
GAME_NAME = VERSION_STRING # same as folder name
...
def uploadResultToS3(bucket,game_folder_name,srcDir):
	...
	print ("URL: https://s3." + BUCKET_LOCATION + ".amazonaws.com/" + BUCKET_NAME + "/" + GAME_NAME + "/index.html")
	print ("Uncached URL: https://s3." + BUCKET_LOCATION + ".amazonaws.com/" + BUCKET_NAME + "/" + GAME_NAME + "/index.html?nocache=1")

def upload(k,b,game_folder_name,path,file,srcDir,language_code):
	print ('Preparing bucket for upload')
	k.key = game_folder_name + "/" + os.path.relpath(os.path.join(path,file),srcDir)
	k.key = re.sub(r'\\', '/', k.key) #added to avoid forward slash in k.key
	print ('sending ' + file + ' to https://s3.' + BUCKET_LOCATION + '.amazonaws.com/engine/'  + b.name + '/' + k.key + ' ...')
