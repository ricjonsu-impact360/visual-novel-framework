...
// @version      1.0.0
// @description  Helper tool for placing and animating objects in visual novel scenes.
// @author       You
...
(function () {
    ...
    function initializeHelperTool () {
        // Use unsafeWindow for Tampermonkey
        ...
        window.ObjectHelperTool = {
            ...
            applyToScene: function () {
                ...
                if (!unsafeWindow.ig || !unsafeWindow.ig.game || !unsafeWindow.ig.game.windowName) {
                    this.showStatus('Game not ready. Make sure you are in the game scene.', 'error');
                    return;
                }

                if (unsafeWindow.ig.game.windowName !== 'game') {
            ...
            preloadImageAndApply: function (objectID, source) {
                ...
                var imagePath = unsafeWindow._BASEPATH.object + source + '.png';

                // Check if image is already in cache and loaded
                if (unsafeWindow.ig.Image.cache[imagePath] && unsafeWindow.ig.Image.cache[imagePath].loaded) {
                ...
                var preloadImage = new unsafeWindow.ig.Image(imagePath);
            ...
            applyObjectToScene: function (objectID, source) {
                ...
                var objectArray = [];
                var currentWindow = unsafeWindow.ig.game.currentWindow;
        ...
        unsafeWindow.beginObjectHelper = function () {
            unsafeWindow.ObjectHelperTool.open();
        };

        unsafeWindow.endObjectHelper = function () {
            unsafeWindow.ObjectHelperTool.close();
        };

        document.addEventListener('keydown', function (e) {
            if (e.ctrlKey && e.shiftKey && e.key === 'O') {
                e.preventDefault();
                if (unsafeWindow.ObjectHelperTool.isOpen) {
                    unsafeWindow.endObjectHelper();
                } else {
                    unsafeWindow.beginObjectHelper();
                }
            }
        });
        ...
        if (_AUTO_SHOW) {
            ...
            setTimeout(function () { unsafeWindow.beginObjectHelper();}, _SHOW_DELAY * 1000);
        }
