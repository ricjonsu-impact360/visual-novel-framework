...
// @version      1.2.1
// @description  Helper tool for testing zoom/pan animation effects in real-time
// @author       You
...
(function () {
    ...
    function initializeHelperTool () {
        ...
        unsafeWindow.ZoomPanHelperTool = {
            isOpen: false,
            helperWindow: null,
            autoApply: false,
        ...
        unsafeWindow.beginZoomPanHelper = function () {
            unsafeWindow.ZoomPanHelperTool.open();
        };

        unsafeWindow.endZoomPanHelper = function () {
            unsafeWindow.ZoomPanHelperTool.close();
        };

        document.addEventListener('keydown', function (e) {
            if (e.ctrlKey && e.shiftKey && e.key === 'Z') {
                e.preventDefault();
                if (unsafeWindow.ZoomPanHelperTool.isOpen) {
                    unsafeWindow.endZoomPanHelper();
                } else {
                    unsafeWindow.beginZoomPanHelper();
                }
            }
        });
        ...
        if (_AUTO_SHOW) {
            ...
            setTimeout(function () { unsafeWindow.beginZoomPanHelper();}, _SHOW_DELAY);
        }
