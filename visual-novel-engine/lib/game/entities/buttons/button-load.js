			...
			clicked:function(){
				if(this.visible && this.isClickable && (ig.game.transition == null || !ig.game.transition.isRunning)) { // && this._parent.enableButton
					...
					if(ig.game.windowName == 'menu'){
						...
						this._parent.uiSaveLoad.show('load');
					} else {
						ig.game.currentWindow.delayClickButton();

						ig.game.pauseGameviaButton(this, 'uiSaveLoad');
			...
			draw:function() {
	            ...
	            if(this.visible && ig.game.showMiniButton) {
					...
					if(_DATAGAME.miniButton.outlineColor != 'none' && _DATAGAME.miniButton.outlineColor != '') {
						...
						c.lineCap = "round";
						c.lineJoin = 'round'; 
						c.strokeText(_STRINGS.Button.load, this.halfSize.x, this.size.y + _DATAGAME.miniButton.textOffset.y*_DATAGAME.ratioRes);
					}
					
					c.fillStyle = _DATAGAME.miniButton.textColor;
					c.fillText(_STRINGS.Button.load, this.halfSize.x, this.size.y + _DATAGAME.miniButton.textOffset.y*_DATAGAME.ratioRes);
				...
				if(ig.game.windowName == 'menu') {
					...
					if(!ig.game.openMiniButton) {
						...
						if(ig.game.currentWindow.isShowingButtonShop) {
							...
							this.pos.y = 25 + this.size.y + (_DATAGAME.miniButton.padding.y+this.size.y)*(ig.game.listMiniButton.indexOf('load') + 1) + _DATAGAME.miniButton.padding.y;
						} else {
							this.pos.y = 25 + this.size.y + (_DATAGAME.miniButton.padding.y+this.size.y)*ig.game.listMiniButton.indexOf('load') + _DATAGAME.miniButton.padding.y;
						}
					}
				}
			}
