ig.module('game.entities.buttons.button-skip')
	.requires(
	...
	.defines(function () {
		EntityButtonSkip = EntityButton.extend({
			...
			imageSkipOn:new ig.Image(_RESOURCESINFO.image.btnSkip2),
			imageSkipOff:new ig.Image(_RESOURCESINFO.image.btnSkip),
			defsize: {
			...
			clicked:function(){
				if(this.visible && this.isClickable && (ig.game.transition == null || !ig.game.transition.isRunning)) { // && this._parent.enableButton
					...
					ig.game.dialogAutoStatus = (ig.game.dialogAutoStatus == ig.game.STAT_DIALOG_SKIP) ? ig.game.STAT_DIALOG_NORMAL : ig.game.STAT_DIALOG_SKIP;
			...
			draw:function() {
	            ...
	            if(this.visible && ig.game.showMiniButton) {
					...
					if(ig.game.dialogAutoStatus == ig.game.STAT_DIALOG_SKIP) this.imageSkipOn.draw(0, 0);
					else  this.imageSkipOff.draw(0, 0);
					...
					if(_DATAGAME.miniButton.outlineColor != 'none' && _DATAGAME.miniButton.outlineColor != '') {
						...
						c.lineCap = "round";
						c.lineJoin = 'round'; 
						c.strokeText(_STRINGS.Button.skip, this.halfSize.x, this.size.y + _DATAGAME.miniButton.textOffset.y*_DATAGAME.ratioRes);
					}

					c.fillStyle = _DATAGAME.miniButton.textColor;
					c.fillText(_STRINGS.Button.skip, this.halfSize.x, this.size.y + _DATAGAME.miniButton.textOffset.y*_DATAGAME.ratioRes);
			...
			repos: function () {
				...
				if(!ig.game.openMiniButton) {
					...
					if(ig.game.currentWindow.isShowingButtonShop) {
						...
						this.pos.y = 25 + this.size.y + (_DATAGAME.miniButton.padding.y+this.size.y)*(ig.game.listMiniButton.indexOf('skip') + 1) + _DATAGAME.miniButton.padding.y;
					} else {
						this.pos.y = 25 + this.size.y + (_DATAGAME.miniButton.padding.y+this.size.y)*ig.game.listMiniButton.indexOf('skip') + _DATAGAME.miniButton.padding.y;
					}
				}
			}
		});
