...
ig.Entity.prototype.tweenBezier = function(path, duration, settings) {
    // progress goes 0→1, but attached to entity so tweens[] works
    this._bezierProgress = 0; 

    var tween = new ig.Tween(this, { _bezierProgress: 1 }, duration, ig.merge({
        easing: settings.easing, //
        onUpdate: function() {
            var t = this._bezierProgress;

            // var px = ig.Tween.Interpolation.Bezier(path.map(p => p.x), t);
            // var py = ig.Tween.Interpolation.Bezier(path.map(p => p.y), t);
			...
			var px = ig.Tween.Interpolation.Bezier(xs, t);
			var py = ig.Tween.Interpolation.Bezier(ys, t);

            // Flexible target variables
            if (settings && settings.target) {
                // Example: { xVar: "circleX", yVar: "circleY" }
                if (settings.target.xVar) this[settings.target.xVar] = px;
                if (settings.target.yVar) this[settings.target.yVar] = py;
            } else {
                // Default: move entity itself
                this.pos.x = px;
                this.pos.y = py;
            }
        }.bind(this)
...
ig.Tween = function(obj, properties, duration, settings) {
	...
	this.update = function() {
		// if(ig.game.isPauseSetting && !this.ignorePause) {
		// 	if(timer && !timer.pausedAt) timer.pause();
		// 	return;
		// } else {
		// 	if(timer && timer.pausedAt) timer.unpause();
		// }
