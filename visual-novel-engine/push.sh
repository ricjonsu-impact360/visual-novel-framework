...
secure_regular (){    
    ...
    echo ""
    python inject_domainlock.py 'domainlock.js' 'engine.js' 'this.START_OBFUSCATION;' 'this.END_OBFUSCATION'
    ...
    echo ""
    javascript-obfuscator 'engine.js' -o 'engine.js' --config 'tools/javascript-obfuscator-dev.json'
    sed -i.bak 's/{data;}else{return;}/{}else{return;}/g' engine.js
    rm *.bak
...
compile_test_game (){
    ...
    echo "Compiling engine.js ..."
    java -jar compiler.jar \
    --warning_level=QUIET \
    --js=media/text/strings.js \
    --js=settings/dev.js \
    --js=lib/game/entities/object/handpart.js \
    --js=lib/game/entities/object/emo_eye.js \
    --js=lib/game/entities/game-setting/datagame.js \
    --js=lib/game/entities/game-setting/resources-info.js \
    ...
    --js=_factory/game/engine.js \
    --js_output_file=engine.js \
    --language_in=ECMASCRIPT5
    echo "Done!"

    echo "Compiling game.css for testing ..."
    bash css-append.sh
    bash css-minify.sh temp.css engine.css
    sed -i.bak 's/..\/..\/..\/..\/..\/..\///g' engine.css
    rm temp.css
    rm *.bak
...
prep_production (){
    ...
    sed -i.bak 's/main.css/engine.css/g' index.html

    # Clean JS paths
    # sed -n '/glue\/jquery\/jquery-3.2.1.min.js/!p' index.html > temp && mv temp index.html
    sed -i.bak 's/glue\/load\/load.js/engine.js/g' index.html
    ...
    echo "Compiling engine.js for _factory ..."
    ...
    then
        ...
        --js=game.min.js \
        --js_output_file=_factory/game/engine.js \
        --language_in=ECMASCRIPT5
    else
        java -jar compiler.jar \
        --warning_level=QUIET \
...
inject_burst_cache_version_tag(){
    ...
    then
        ...
        sed -i.bak 's/engine.js/engine.js?'$CACHE_BURST_VERSION_TAG'/g' index.html
        sed -i.bak 's/engine.css/engine.css?'$CACHE_BURST_VERSION_TAG'/g' index.html
        rm *.bak
        echo "Done!"
        echo ""
    fi
